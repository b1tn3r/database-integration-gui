...
public class InsertFormController implements Initializable {
    ...
    private void addToDatabase(ActionEvent event) {
        try {
            String lastName = EncryptionAPI.getInstance().filterInput(EncryptionAPI.getInstance().filterString(lastNameField.getText()));
            String firstName = EncryptionAPI.getInstance().filterInput(EncryptionAPI.getInstance().filterString(firstNameField.getText()));
            String position = EncryptionAPI.getInstance().filterInput(EncryptionAPI.getInstance().filterString(positionField.getText()));
            String address = EncryptionAPI.getInstance().filterInput(EncryptionAPI.getInstance().filterString(addressField.getText()));
            String city = EncryptionAPI.getInstance().filterInput(EncryptionAPI.getInstance().filterString(cityField.getText()));
            String phone = EncryptionAPI.getInstance().filterInput(EncryptionAPI.getInstance().filterString(homePhoneField.getText()));         // all variables set from their text fields values except for file, which needs to be taken from a filechooser

            File file = new File(EncryptionAPI.getInstance().filterFile(EncryptionAPI.getInstance().filterString(photoField.getText())));         // a new file is made from the path given to the photoField from the fileChooser
